...
modelVec = ImgVector2(3, 256, 7, 32, 1).to(device)
lossFn = nn.MSELoss()
optim = Adam(modelVec.parameters(), args.learning_rate)
scheduler = MultiStepLR(optim, milestones=[25, 50, 75, 100],gamma=0.1)
...
def train():
    ...
    for e in range(1, args.epochs+1):
        ...
        with torch.no_grad():
            ...
            print(f"The Training loss after Epoch{e} is {sum(Tloss)/len(Tloss): 0.5f}", "\n")
    ...
    im1 = axs1.imshow(weight[:, :-7], cmap="bwr")
    plt.colorbar(im1)

    im2 = axs2.imshow(weight[:, -7:], cmap = "bwr")
    plt.colorbar(im2)
